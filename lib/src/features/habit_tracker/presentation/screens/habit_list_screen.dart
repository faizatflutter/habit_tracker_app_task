    ...
    return BlocListener<ConnectivityCubit, NetworkStatus>(
      listener: (context, state) {
        if (state == NetworkStatus.disconnected) {
          context.showSnackBarMessage(AppStrings.noInternetConnection, isError: true);
        }
      },
      child: Scaffold(
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () {
            final isConnected = context.read<ConnectivityCubit>().state == NetworkStatus.connected;
            if (isConnected) {
              _openAddHabitBottomSheet(context);
            } else {
              context.showSnackBarMessage(AppStrings.noInternetConnection, isError: true);
            }
          },
